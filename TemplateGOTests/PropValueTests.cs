    ...
    public class PropValueTests
    {
        ...
        public void ValueTest()
        {
            ...
            using (var doc = SpreadsheetDocument.Open(outFile, false))
            {
                ...
                cell = sheet1Part.Worksheet.Descendants<Cell>().Where(cell => cell.CellReference == "B13").FirstOrDefault();
                Assert.IsNotNull(cell);
                Assert.IsTrue(CellValues.String == cell.DataType!, cell.CellReference);
                Assert.AreEqual("我得了46.5分", cell.InnerText, cell.CellReference);

                cell = sheet1Part.Worksheet.Descendants<Cell>().Where(cell => cell.CellReference == "B15").FirstOrDefault();
                Assert.IsNotNull(cell);
                Assert.IsTrue(CellValues.String == cell.DataType!, cell.CellReference);
                ...
                cell = sheet1Part.Worksheet.Descendants<Cell>().Where(cell => cell.CellReference == "B21").FirstOrDefault();
                Assert.IsNotNull(cell);
                Assert.IsTrue(CellValues.String == cell.DataType!, cell.CellReference);
